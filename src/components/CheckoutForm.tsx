...
export const CheckoutForm = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await fetch('https://j98qnay8nk.us.aircode.run/payment', {
      method: 'POST',
      headers: {
  ...
  return (
    <form onSubmit={handleSubmit} className='px-4'>
      <div className='mb-3'>
        ...
        <div>
          <input value={emailInput} onChange={(e => setEmailInput(e.target.value))} id='email-input'type='email' placeholder='JohnDoe@gmail.com' className='p-3 w-full bg-[#fff] rounded-[5px] border border-[#e6e6e6] box-shadow-custom'/>
        </div>
      </div>
      <PaymentElement />
